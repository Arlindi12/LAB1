...
const Users = () => {
  ...
  const [user, setUser] = useState({userId: 0, firstname: '', lastname: '', email: '', password: '', roleId: 0 });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setUser({ userId: 0, firstname: '', lastname: '', email: '', password: '', roleId: 0 });
      loadUsers();
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await API.delete(`/Account/DeleteUser/${selectedUser.userId}`);
